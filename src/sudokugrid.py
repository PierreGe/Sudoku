...
class SudokuGrid():
    def __init__(self, grid):
        ...
        self._conflictPositionDict = dict((s, set(sum(dict((s, [e for e in [SudokuGrid.combination(self._letters, d) for d in self._digits] + [SudokuGrid.combination(l, self._digits) for l in self._letters] + [SudokuGrid.combination(rs, cs) for rs in self._squares[0] for cs in self._squares[1]] if s in e]) for s in [l+d for l in self._letters for d in self._digits])[s],[]))-set([s])) for s in [l+d for l in self._letters for d in self._digits])
        self._grid = {}
        self.parse(grid)
